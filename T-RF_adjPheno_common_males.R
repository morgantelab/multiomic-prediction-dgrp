...
for(rep in 1:nReps){
  ...
  for(fold in 1:folds){
    ...
    yTrain <- yM[-whichNa]
    xTrain <- dataMSel[-whichNa, ]
    yTest <- yM[whichNa]
    xTest <- dataMSel[whichNa, ]
...
###Write to a file
write.table(Results_final, paste('Results/Pred_Expression_RandomForest_adjPheno_common_', colnames(pheno)[trait],'_males.csv', sep=""), 
