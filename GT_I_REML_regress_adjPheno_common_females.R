...
###GREML
fm_G <- regress(yF ~ 1, ~GRM, pos=c(T, T), maxcyc=10000)
Vg_G <- fm_G$sigma[1]
Vt_G <- as.numeric(NA)
Vi_G <- as.numeric(NA)
Ve_G <- fm_G$sigma[2]
...
###TREML
fm_T <- regress(yF ~ 1, ~TRM, pos=c(T, T), maxcyc=10000)
Vg_T <- as.numeric(NA)
Vt_T <- fm_T$sigma[1]
Vi_T <- as.numeric(NA)
Ve_T <- fm_T$sigma[2]
...
###GTREML
fm_GT <- regress(yF ~ 1, ~GRM+TRM, pos=c(T, T, T), maxcyc=10000)
Vg_GT <- fm_GT$sigma[1]
Vt_GT <- fm_GT$sigma[2]
Vi_GT <- as.numeric(NA)
Ve_GT <- fm_GT$sigma[3]
...
###GT-I-REML
fm_GTI <- regress(yF ~ 1, ~GRM+TRM+IRM, pos=c(T, T, T, T), maxcyc=10000)
...
###Write to a file
write.table(Results_final, paste('Results/Variance_Partition_SNPs_Expression_Interaction_regress_adjPheno_common_', colnames(pheno)[trait],'_females.csv', sep=""), 
