...
for(i in c(3, 9, 10)){
	...
	gPropVarPart_starv <- pPropVarPart_starv + geom_bar(stat = "identity") + 
	  ...
	  theme_bw() +
	  theme(axis.text=element_text(size=16), axis.title=element_text(size=18), axis.text.x=element_text(angle=45, hjust=1),
	...
	##Save to a pdf
	pdf(paste('Results/Graphs/Manuscript/Proportion_Variance_Partition_TrainingSet_GRM_TRM_IRM_adjPheno_common_', trait, '_MS.pdf', sep=""), width=15, height=11)
