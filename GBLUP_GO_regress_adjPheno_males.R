...
complete_res <- foreach(i=1:length(go2variants), .combine='rbind', .packages='regress') %dopar% {
	...
	Mat <- go2GRMs(GO_num=i, GO_data=go2variants, W_data=W, phenoLines=names(yM))
...
write.table(complete_res, paste('Results/Pred_SNPs_GO_regress_adjPheno_', colnames(pheno)[trait],'_males.csv', sep=""), 
