...
common <- intersect(phenoSel[, 1], row.names(dataM))
...
dataMSel <- dataM[names(yM), ]

###Scale the transcript data
dataMSel <- scale(dataMSel, center=TRUE, scale=TRUE)
...
for(rep in 1:nReps){
  ...
  for(fold in 1:folds){
    ...
    ##TWAS    
    Out <- mclapply(1:ncol(dataMSel), mylmm, dataMSel, yNa, mc.cores=cores)
    
    ##Organize results
    twas_res <- unlist(Out)
	names(twas_res) <- colnames(dataMSel)
    ...
    if(length(sig_tran)>0){
    	dataMSel_sig <- dataMSel[, colnames(dataMSel) %in% sig_tran]
    	
    	if(length(sig_tran)==1){dataMSel_sig <- matrix(dataMSel_sig, ncol=1)}
    
    	###Compute TRM
		TRM <- tcrossprod(dataMSel_sig)/ncol(dataMSel_sig)
...
###Write to a file
write.table(Results_final, paste('Results/Pred_Expression_TWAS_LMM_pvalue_', pval, '_regress_adjPheno_common_', colnames(pheno)[trait],'_males.csv', sep=""), 
