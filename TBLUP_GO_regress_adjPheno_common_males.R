...
## Fabio Morgante
## 7/13/2017
## TBLUP with GO annotation
##
## Modified on 4-23-2018
## Reason: adjust phenotypes, calculate h2 e2 and pool folds and reps to calculate se
##
## Modified on 5-3-2018
## Reason: Get transcript data for only lines with phenotype
## 
...
###Load the data
##Annotations
load("Annotations/go2fb_M.RData")
                              
##Centered and scaled transcript matrix (W_M)
load(file="Matrices/dgrp2_scaled_transcripts_common_males.RData")
                              
##Phenotypes
pheno <- read.csv("Data/eQTL_traits_males.csv", header=TRUE)

###Extract GOs with at least 5 genes
go2fb_M <- go2fb_M[sapply(go2fb_M,length)>=5]
...
common <- intersect(phenoSel[, 1], row.names(W_M))
...
###Loop across GOs
complete_res <- foreach(i=1:length(go2fb_M), .combine='rbind', .packages='regress') %dopar% {

	###Compute TRMs and store matrices in a list needed in the prediction function
	Mat <- go2TRMs(GO_num=i, GO_data=go2fb_M, W_transc_data=W_M, phenoLines=names(yM))
	...
	for(rep in 1:nReps){
  		...
  		for(fold in 1:folds){
    		...
    		fm <- tryCatch(regress(yNa ~ 1, ~TRM_GO+TRM_notGO, pos=c(T, T, T), maxcyc = 10000), error=function(e){message(paste0("Error in GO = ", i, ", CV = ", rep, ", fold = ", fold, ":\n", e)); return(NULL)}, 
...
write.table(complete_res, paste('Results/Pred_Expression_GO_regress_adjPheno_common_', colnames(pheno)[trait],'_males.csv', sep=""), 
