...
###Extract common GOs
common_GOs <- intersect(names(go2fb_M), names(go2variants))
go2fb_M <- go2fb_M[common_GOs]
...
complete_res <- foreach(i=1:length(go2fb_M), .combine='rbind', .packages='regress') %dopar% {
	...
	Mat1 <- go2TRMs(GO_num=i, GO_data=go2fb_M, W_transc_data=W_M, phenoLines=names(yM))
	Mat2 <- go2GRMs(GO_num=i, GO_data=go2variants, W_data=W,  phenoLines=names(yM))
	Mat <- c(Mat1, Mat2)
	rm(Mat1, Mat2)
...
write.table(complete_res, paste('Results/Pred_SNPs+Expression_GO_regress_adjPheno_common_', colnames(pheno)[trait],'_males.csv', sep=""), 
