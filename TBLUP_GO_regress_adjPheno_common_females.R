...
complete_res <- foreach(i=1:length(go2fb_F), .combine='rbind', .packages='regress') %dopar% {
	...
	Mat <- go2TRMs(GO_num=i, GO_data=go2fb_F, W_transc_data=W_F, phenoLines=names(yF))
...
write.table(complete_res, paste('Results/Pred_Expression_GO_regress_adjPheno_common_', colnames(pheno)[trait],'_females.csv', sep=""), 
