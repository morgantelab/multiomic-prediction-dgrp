...
for(i in 10){
	...
	scatter <- p + geom_point(size=1) +  geom_smooth(method="lm", colour="black") +
	  stat_cor(method = "pearson", cor.coef.name ="r", label.x=50000, size=3.7) +
	...
	##Save to a pdf
	jpeg(paste('Results/Graphs/Manuscript/Accuracy_vs_GOsize_GO-SNPs_adjPheno_', trait,'_MS.jpeg', sep=""), width=20, height=15, units="cm", res=300)
	print(scatter)
	dev.off()
