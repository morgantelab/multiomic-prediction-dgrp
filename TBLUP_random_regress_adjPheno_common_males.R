...
for(rep in 1:nReps){
  ...
  for(fold in 1:folds){
    ...
    rnd_tran <- sample(colnames(dataMSel), numTrans, replace=F)
   	dataMSel_rnd <- dataMSel[, colnames(dataMSel) %in% rnd_tran]
    
   	###Compute TRM
	TRM <- tcrossprod(dataMSel_rnd)/ncol(dataMSel_rnd)
...
###Write to a file
write.table(Results_final, paste('Results/Pred_Expression_Random_', numTrans, '_regress_adjPheno_common_', colnames(pheno)[trait],'_males.csv', sep=""), 
