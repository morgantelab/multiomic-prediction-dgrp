...
accuracies <- read.csv(paste('Results/Pred_SNPs_GO_regress_adjPheno_', colnames(pheno)[trait],'_females.csv', sep=""), header=TRUE)
...
write.table(overlap_matrix, paste('Results/Overlap_SNPs_gene_level_top3GO_regress_adjPheno_common_', colnames(pheno)[trait],'_females.csv', sep=""), 
...
###Write results to a file
write.table(overlap_variants_matrix, paste('Results/Overlap_SNPs_variant_level_top3GO_regress_adjPheno_common_', colnames(pheno)[trait],'_females.csv', sep=""), 
		  col.names=T, row.names=F, quote=F, sep=",")
